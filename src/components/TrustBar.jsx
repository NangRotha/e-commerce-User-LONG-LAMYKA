...
export default function TrustBar() {
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 pt-4 pb-2">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4 p-4 sm:p-5 rounded-3xl bg-white/70 dark:bg-slate-900/70 backdrop-blur-xl border border-slate-200/80 dark:border-slate-800 shadow-soft">
        {items.map((item, idx) => (
          <div
            key={idx}
            className="flex items-center gap-3.5 p-2 sm:p-2.5 rounded-2xl hover:bg-slate-50 dark:hover:bg-slate-800/60 transition-colors group"
          >
            <div
              className={`w-11 h-11 rounded-2xl ${item.bg} flex items-center justify-center shrink-0 shadow-sm transition-transform duration-300 group-hover:scale-110`}
            >
